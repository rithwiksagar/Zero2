    ...
    <div className="flex-1 overflow-y-auto [scrollbar-width:thin] [scrollbar-color:var(--color-zinc-500)_transparent]">
      {filteredModels.map((model) => (
        <div key={model.id} className="">
          <span className="text-neutral-400 text-[14px] ml-2.5">
            {model.chef}
          </span>
          <div className="flex flex-col">
            {model.names.map((modelName) => (
              <div
                key={modelName}
                className={`text-neutral-50 text-[14px] pl-6 pr-2 py-1 cursor-pointer hover:bg-neutral-800/80 rounded-md flex justify-between items-center ${defaultModel === modelName ? "bg-neutral-800/40" : ""}`}
                onClick={() => {
